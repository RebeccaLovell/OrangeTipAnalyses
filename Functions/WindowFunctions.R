...
### Functions for running sliding window analyses ###
...
GetWindowsClimates<-function(SpeciesName, AbundanceData, CountPercentCurr, CountPercentPrev,
  ...
  # find the day we're working from, when a given % of counts have been made
  ...
  for(i in 1:nrow(ClimWindows)){
    ...
    # find Spatialav, the mean for each site across years 

    if(is.na(SpAvSubset[1])){ # if no spatialav subset years
    ...
    }else{ # if spatailav subset years
...
# in RunSlidingWindow can specify eg Rows=c(1,50) or Rows=c(200,"end") in the function to run specific rows to allow parallel running of models
...
RunSlidingWindow<-function(SpeciesName, FocWindow, LastWindow=FALSE, 
  ...
  # set up data frame for all windows and their outputs
  
  WindowModels<-data.frame(windows=ClimWindows$window) 
  ...
  if(SpaceOnly==FALSE&&TimeOnly==FALSE){ # full model
  ...
  if(SpaceOnly==TRUE){ # space only model
  ...
  if(TimeOnly==TRUE){ # time only model
    WindowModels[,(ncol+1):(ncol+4)]<-NA
  ...
  if(StartVals==TRUE){
    ...
    # row for best model from previous run
    bestrow<-which(PrevWindowModels$AIC==min(PrevWindowModels$AIC,na.rm=T))
    
    # specify formula for the best model from previous run, this will be used to get start values
    ...
    if(TimeOnly==TRUE){
      ...
      if(class(bestmod)=="try-error"){ # if error in fitting, try a different optimiser
  ...
  for(focrow in RowsToRun){
    ...
    # input model estimates into data frame
  ...
  if(nrow(WindowModels)==nwindows){ # if all models have run (or attempted to run) and saved, find the best
    ...
    # save models
    # this will be overwritten with each new climate variable explored
...
# function to plot the sliding window outputs [crude initial plots]

PlotWindows<-function(SpeciesName, FocWindow, Scaled=FALSE, CountPercentCurr, CountPercentPrev,
                      WindowDurations, WindowStartInterval, SpAvSubset=NA, AR1=FALSE, AR1YearDev=FALSE,
                      SpaceOnly=FALSE,TimeOnly=FALSE,StartVals=FALSE){
  ...
  foccol<-grep(paste0("Window",FocWindow),colnames(WindowModels))
  
  Starts<-ClimWindows$WindowStartDay[match(WindowModels[,foccol], ClimWindows$window)]
  Ends<-ClimWindows$WindowEndDay[match(WindowModels[,foccol], ClimWindows$window)]
  
  # find the best window days
  bestWindow<-WindowModels[which(WindowModels$AIC==min(WindowModels$AIC,na.rm=T)),foccol]
  bestStart<-ClimWindows$StartDate[which(ClimWindows$window==bestWindow)]
  bestEnd<-ClimWindows$EndDate[which(ClimWindows$window==bestWindow)]
  bestStartOrd<-ClimWindows$StartOrdinalDay[which(ClimWindows$window==bestWindow)]
  bestEndOrd<-ClimWindows$EndOrdinalDay[which(ClimWindows$window==bestWindow)]

  pdf(file=paste0(modfolder,SpeciesName,"_SlidingWindowPlots_",FocWindow,modname,if(StartVals==TRUE){paste0("_StartVals")},".pdf"), width = 4, height = 8)

  par(mfrow=c(2,1))
  
  # plot windows AICs
  plot(0,0,type="n", ylab="AIC",xlab="Days prior",main=paste0(FocWindow, " AIC"), 
       ylim=range(WindowModels$AIC,na.rm=T),xlim=c(nrow(DayNumbers),1))
  axis(1, at=c(1,nrow(DayNumbers)), labels=DayNumbers$Date[match(c(1,nrow(DayNumbers)),DayNumbers$DaysPrior)],line =1, lty=0)
  segments(y0=WindowModels$AIC,
           x0=Starts, 
           x1=Ends,
           col=rgb(0,0,0,0.5))
  abline(h=min(WindowModels$AIC,na.rm=T)+2,lty=2,col="red")
  
  mtext(paste0("Best = Ordinal ", bestEndOrd,"-",bestStartOrd, 
               "(", bestEnd,"-",bestStart,")",
               "MinAIC=", min(WindowModels$AIC,na.rm=T)),side=3,cex=0.75)
  
  ### plot log likelihood
  plot(Starts,WindowModels$logLik,xlim=c(nrow(DayNumbers),1),xlab="Days prior",col=rgb(1,0,0,0.5),
       pch=16,cex=0.5,main=paste0(FocWindow," LogLikelihood"))
  points(Ends,WindowModels$logLik,col=rgb(0,0,1,0.5),pch=16,cex=0.5)
  axis(1, at=c(1,nrow(DayNumbers)), labels=DayNumbers$Date[match(c(1,nrow(DayNumbers)),DayNumbers$DaysPrior)],line =1, lty=0)
...
# do model checking on the best model 

# AIC range across windows
# correlations between climate variables
# check for zero inflation
# qq plots
# run different optimisers 

CheckWindowModels<-function(SpeciesName, FocWindow, Scaled=FALSE, CountPercentCurr, CountPercentPrev,
                            WindowDurations, WindowStartInterval, SpAvSubset=NA, AR1=FALSE, AR1YearDev=FALSE,
                            SpaceOnly=FALSE,TimeOnly=FALSE, StartVals=FALSE){
  ...
  # identify best model
  WindowModels<-WindowModels[order(WindowModels$AIC),]
  focrow<-which(WindowModels$AIC==min(WindowModels$AIC,na.rm=T))
  
  print(paste0(c("AIC Range", (max(WindowModels$AIC,na.rm=T)-min(WindowModels$AIC,na.rm=T)))))
  
  # specify if we are using scaled variables or not
  if(Scaled==TRUE){
    MaxTempVar<-"MaxTempScaled"
    PrecipVar<-"PrecipScaled"
  }else{
  ...
  if(length(tempcols)>0&&length(precipcols)>0){
    t<-AbundanceData[,paste0(MaxTempVar,"_",WindowModels[focrow,tempcols])]
    p<-AbundanceData[,paste0(PrecipVar,"_",WindowModels[focrow,precipcols])]
    
    cor<-cor.test(t,p)
    print(cor)
    #plot(t,p,main=paste0("cor=", cor$estimate))
  }
  
    # specify best model formula from table and run model
      ...
      # run model
      focmod<-try(glmer(focformula,family=poisson,data=AbundanceData,na.action=na.omit))
      
      # if there was an error, try another optimiser
      if(class(focmod)=="try-error"){
        focmod<-glmer(focformula,family=poisson,data=AbundanceData,na.action=na.omit,glmerControl(optimizer="bobyqa"))
  ...
  print(summary(focmod))
  
  # identify REs with sd of 0
  SDZero<-as.data.frame(summary(focmod)$varcor)[which(as.data.frame(summary(focmod)$varcor)[,"sdcor"]<0.00001),"grp"]
  print(paste0("REs with sd 0 = ", SDZero))
  save(SDZero, file=paste0(modfolder,SpeciesName,"_REsExclude",modname,FocWindow,".rda"))

  pdf(paste0(modfolder,SpeciesName,"_SlidingWindowModelChecks",FocWindow,modname,if(StartVals==TRUE){paste0("_StartVals")},".pdf"),width=10,heigh=7.5)
  par(mfrow=c(1,2))
  
    ### Check for zero inflation
    sims<-simulate(focmod, 1000) 
    nzerosim<-apply(sims,2, function(x) length(which(x==0)))
    hist(nzerosim, main=paste(SpeciesName, FocWindow))
    abline(v=length(which(AbundanceData$SITE.INDEX==0)),col="red")    
    abline(v=mean(nzerosim)+(2*sd(nzerosim)),col="blue")
    abline(v=mean(nzerosim)-(2*sd(nzerosim)),col="blue")
    mtext(paste0("observed=",length(which(AbundanceData$SITE.INDEX==0))), col="red")
    
    ### qq plot
    sims <- simulateResiduals(fittedModel = focmod, plot = F)
    plotQQunif(sims)
    
    dev.off()

   # fit this model with different optimisers 
      #allfit<-allFit(focmod)
      
      focmod_default<-glmer(focformula,family=poisson,data=AbundanceData,na.action=na.omit)
      focmod_bobyqa<-glmer(focformula,family=poisson,data=AbundanceData,na.action=na.omit,glmerControl(optimizer="bobyqa"))
      
    # do estimates vary among optimisers?
    
      # log likelihood 
      focrange<-as.numeric(summary(focmod_default)$logLik)-as.numeric(summary(focmod_bobyqa)$logLik)
      print(paste("logLik diff ",round(focrange,3)))

      # fixed effects 
      for(i in 1: nrow(summary(focmod_default)$coef)){
        focrange<-summary(focmod_default)$coef[i,1]-summary(focmod_bobyqa)$coef[i,1]
        print(paste(rownames(summary(focmod_default)$coef)[i], "diff", round(focrange,3)))
      }
      
      # RE SDs 
      for(i in 1: nrow(as.data.frame(summary(focmod_default)$varcor))){
        focrange<-as.data.frame(summary(focmod_default)$varcor)[i,"sdcor"]-as.data.frame(summary(focmod_bobyqa)$varcor)[i,"sdcor"]
        print(paste("RE SD", as.data.frame(summary(focmod_default)$varcor)[i,"grp"], "diff",round(focrange,3)))
      }
    }

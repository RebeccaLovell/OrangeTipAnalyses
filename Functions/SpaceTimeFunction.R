...
### functions for comparing climate effects over space and time ###
...
# model to run space time comparison analyses to compare the effect of climate variables over space and time 
...
SpaceTimeModel<-function(SpeciesName, Hurdle=FALSE, Scaled=FALSE, exclude=NA, AR1=FALSE,AR1YearDev=FALSE,  
  ...
  # if different from the space v time analyses, what previous year abundance predictor was used in the windows search? 
  ...
  # specify folder and file names, dependent on function arguments
  ...
  randomef<-c("Site","Year","GridCell50km","GridCell50km:Year") # full set of random effects (REs)
  
  if(REsFiltered==TRUE){  # if filtering REs, load REs to exclude (those with var zero in window search)
  if(any(Vars=="Precip")){ # if precipitation or temperature and precipitation model, load the REs to exclude based on the precipitation window search (this search also includes any identified temp window)
    ...
    if(length(SDZero)>0){randomef<-randomef[-match(SDZero,randomef)]} # remove REs with sd of zero
  }
  
  if(WindowAR1Diff==TRUE && AR1YearDevWindows==FALSE){ # if AR1 yeardev wasn't calculated for windows search, calculate it now
  ...
  # find columns for variables of interest in windows output
 ...
 # get windows names
  ...
  # fit space v time model
  
  if(Hurdle==FALSE){ # if not a hurdle model
    
  # parameter expanded priors
    # G is random effects, R is residuals, fixed effects are B
    # default priors used for fixed effects
    # 'if' clauses here are to set priors for the correct number of REs, depending on the number included
    
    a<-1000
  if(length(randomef)==4){ # if 4 REs
  ...
  pa_prior<-list(R=list(V=diag(1), nu=0.002), 
    ...
    if(length(randomef)==2){ # if 2 REs
      pa_prior<-list(R=list(V=diag(1), nu=0.002), 
                     G=list(G1=list(V=diag(1), nu=1, alpha.mu=0, alpha.V=diag(1)*a),
                            G2=list(V=diag(1), nu=1, alpha.mu=0, alpha.V=diag(1)*a)))
    }
    
    if(length(randomef)==1){ # if 3 REs
      pa_prior<-list(R=list(V=diag(1), nu=0.002), 
                     G=list(G1=list(V=diag(1), nu=1, alpha.mu=0, alpha.V=diag(1)*a)))
    }
  ...
  # specify climate predictors based on windows identified
  predictors<-paste0("`",Windows,"SpatialAv`*`",Windows,"YearDev`+I(`",Windows,"SpatialAv`^2)") # adding `` as var names have numbers in
  
  if(AR1==TRUE){ # if previous year abundance predictor, add this to fixed effects
    if(AR1YearDev==TRUE){
  ...
  # run model
    spacetimemod<-MCMCglmm(fixed=fixedeffs, random = as.formula(paste0("~ ",paste0(randomef,collapse = "+"))), 
                            family="poisson", data = AbundanceData, nitt=nitt, thin=thin, burnin=(nitt/5),
                           prior=pa_prior, pr=TRUE)
  ...
  if(Hurdle==TRUE){ # hurdle poisson model
    ...
    # 'if' clauses here are to set priors for the correct number of REs, depending on the number included
      ...
      pa_prior<-list(R=list(V=diag(2), nu=0.002, fix=2), # binomial bit is binary, so cant estimate a residual and have to fix it  
      ...
      # run model
        spacetimemod<-MCMCglmm(fixed=fixedeffs, random = as.formula(paste0("~ ", paste0(paste0("us(trait):",randomef) ,collapse="+"))), rcov=~idh(trait):units, 
                               family="hupoisson", data = AbundanceData, nitt=nitt, thin=thin, burnin=(nitt/5),
                               prior=pa_prior, pr=TRUE)
        # us = unstructured - allows covariance between binomial and truncated-poisson parts of the model
        # can't estimate covariance for residuals, so these have idh structure
...
CheckSpaceTimeModel<-function(SpeciesName, Hurdle=FALSE, Scaled=FALSE, exclude=NA, AR1=FALSE,AR1YearDev=FALSE,
  ...
  # load sliding window outputs and abundance data 
  ...
  pdf(paste0(modfolder,SpeciesName,"_SpaceTimeModelChecks",modname,"_",nitt,"itt.pdf"),width=10,heigh=7.5)
  par(mfrow=c(1,2))
  
    ## Check for zero inflation
    sims<-simulate(spacetimemod, 1000) 
    nzerosim<-apply(sims,2, function(x) length(which(x==0)))
    hist(nzerosim, main=paste(SpeciesName, modname))
    ...
    ## traceplots 
    pdf(paste0(modfolder,SpeciesName,"_SpaceTimeModelTraceplots",modname,"_",nitt,"itt.pdf"),width=10,height=10)
    plot(spacetimemod)
    dev.off()
...
#########################################
  ...
  CompareSpaceTime<-function(SpeciesName, Hurdle=FALSE, Scaled=FALSE, exclude=NA, AR1=FALSE, AR1YearDev=FALSE,
    ...
    # find the peak ('optimum') in space and time for each window
    ...
    if(Hurdle==TRUE){ # if hurdle model
      ...
      # find optimum from truncated poisson and binomial parts of the model 
    ...
    for(i in 1:length(Windows)){ # for each window
      ...
      # want to plot when other variable's spatial climate value is held at the mean
      ...
      # get the spatial response. This is intercept + spatial + spatial^2
      spatialresponse<-c()
      ...
      # find the yeardev response at different spatial climate values
      
      for(focspatialclim in spatialvals){
        
        # here, include spatial and yeardev effects (intercept + spatial + spatial^2 + temporal + spatial:temporal)
        # this is because you're looking at the deviation from a specific spatial climate value
        # the spatial value is held constant while yeardev changes  
        ...
        # add yeardev line (for this spatial climate value) to plot  
        # here, yeardevdummy is added to spatial value as it's the deviation not the absolute temp value
        lines((focspatialclim+yeardevdummy), yeardevresponse,col="red")  
        
      }
      
      # add lines where space and time peak 
    ...
    # spatial climate values to find posteriors at
    min<-round(min(AbundanceData[,paste0(focwindow,"SpatialAv")]),1)
    max<-round(max(AbundanceData[,paste0(focwindow,"SpatialAv")]),1)
    ...
    # plot focal climate value vs slope difference  
    ...
    # plot focal climate value vs spatial slope 
    ...
    # plot focal climate value vs temporal slope 
    ...
    if(Hurdle==TRUE){ 
      ...
      for(i in 1:length(Windows)){ # for each window
        ...
        for(focspatialclim in spatialvals){
          ...
          # add yeardev line (for this spatial climate value) to plot  
          # here, yeardevdummy is added to the spatial value as it's the deviation not the absolute temp value
      ...
      for(i in 1:length(Windows)){
        ...
        # plot focal clim vs slope difference for poisson
        ...
        # plot focal clim vs spatial slope for poisson
        ...
        # plot focal clim vs temporal slope for poisson
      ...
      # plot focal clim vs slope difference for binomial
      ...
      # plot focal clim vs spatial slope for binomial
      ...
      # plot focal clim vs temporal slope for binomial
